...
// projectHeader is the HTTP header the client sets to identify the project
// the request is for. The value must be "<projectSlug>:<logType>" —
// PerSignalResourceDef splits it on the first ":" and produces one resolved
// resource per signal present in the v5 query body, with id
// "<projectSlug>:<signal>:<logType>". The middleware authorizes
// per-(project, signal, logType).
const projectHeader = "X-Signoz-Project"

func (provider *provider) addQuerierRoutes(router *mux.Router) error {
	if err := router.Handle("/api/v5/query_range", handler.New(
		provider.authzMiddleware.CheckResources(
			provider.querierHandler.QueryRange,
		...
		handler.WithResourceDefs(handler.PerSignalResourceDef{
			Verb:             coretypes.VerbRead,
			Category:         coretypes.ActionCategoryDataAccess,
			ProjectHeader:    projectHeader,
			SignalsExtractor: coretypes.V5QuerySignalsExtractor(),
			Selector:         coretypes.ProjectLogTypeSelector,
		}),
	)).Methods(http.MethodPost).GetError(); err != nil {
		return err
	}

	if err := router.Handle("/api/v5/substitute_vars", handler.New(
		provider.authzMiddleware.CheckResources(
			provider.querierHandler.ReplaceVariables,
			authtypes.SigNozAdminRoleName,
			authtypes.SigNozEditorRoleName,
			authtypes.SigNozViewerRoleName,
		),
